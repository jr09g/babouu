class SpendingTrendsController < ApplicationController
    ...
    @biz_users_pre = User.joins(:relationship, :receipts, :receipt_items).where("receipts.plain_date" => @this_year).where("relationships.business_id" => current_business.id).where.not("receipts.expense_report_id" => 4)
    @biz_users = @biz_users_pre.select("receipt_items.id, receipt_items.category, receipt_items.price").distinct
