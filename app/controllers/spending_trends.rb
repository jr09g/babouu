class SpendingTrendsController < ApplicationController
    ...
    @biz_users_pre_pre = User.joins(:relationship, :receipt_items).where("relationships.business_id" => current_business.id)#.where.not("receipts.expense_report_id" => 4)
    @biz_users = @biz_users_pre_pre.joins(:receipts).select("receipts.id as receipt_id, receipt_items.id as receipt_item_id, receipt_items.price as receipt_item_price")
