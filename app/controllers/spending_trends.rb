class SpendingTrendsController < ApplicationController
    ...
    @biz_users = User.joins(:relationship, :receipts, :receipt_items).where("receipts.plain_date" => @this_year).where("relationships.business_id" => current_business.id).where.not(:expense_report_id => nil)
